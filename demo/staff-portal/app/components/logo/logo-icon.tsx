...
const LogoIconSvg = ({ width, className, theme }: LogoIconSvgProps) => {
  const { base, icon } = logoStyles({ theme });
  ...
  return (
    <svg
      ...
      <path className={icon()} d={LOGO_PATH} />
    </svg>
  );
};
...
export const LogoIcon = ({ width = 385, className }: LogoProps) => {
  ...
  return (
    <ClientOnly fallback={<LogoIconSvg width={width} className={className} theme="light" />}>
      <LogoIconSvg width={width} className={className} theme={theme} />
    </ClientOnly>
  );
};
